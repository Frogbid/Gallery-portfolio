...
jQuery(document).ready(function ($) {
    ...
    $('.dt-sc-toggle-frame-set').each(function () {
        var $this = $(this), $toggle = $this.find('.dt-sc-toggle-accordion');
    ...
    $('.menu-trigger').on('click', function (e) {
        ...
        if ($('.menu-trigger').parents('.header').hasClass('nav-is-visible')) $('#header').css({top: 0}).stop().animate({top: '100px'}, 200); else $('#header').stop().animate({top: 0}, 600);
        e.preventDefault();
    });
    ...
    $(window).load(function () {
        ...
        if ($(".apply-isotope").length) {
            $(".apply-isotope").isotope({
                ...
                masonry: {gutterWidth: 14}
            });
        }
        $('.blog .post-next-link a').on("click", function (e) {
            var $this = $(this), $fetch_page = parseInt($this.attr('data-page'), 10) + 1;
            $.ajax({
                type: "POST",
        ...
        $(function () {
            $('.shop-dropdown').each(function () {
                $(this).dropdown({gutter: 4, stack: false, delay: 100, slidingIn: 58});
            });
        });
        if ($('.blog-slider').length) {
            $('.blog-slider').bxSlider({auto: true, mode: 'fade', pager: '', adaptiveHeight: true, autoHover: true});
        }
        if ($('.blog-slider').length) {
            $('.blog-slider').bxSlider({auto: false, mode: 'fade', pager: '', adaptiveHeight: true, autoHover: true});
        }
        if ($(".recent-gallery").find("li").length > 1) {
            $(".recent-gallery").bxSlider({
                auto: true,
                useCSS: false,
        ...
        (function ($) {
            $(".dt-sc-progress").one('inview', function (event, visible) {
                var $this = $(this), pvalue = $this.find('.dt-sc-bar').attr('data-value');
                if (visible == true) {
                    $this.find('.dt-sc-bar').animate({width: pvalue + "%"}, 600, function () {
        ...
        if ($map.length) {
            $map.gMapResp({
                ...
                markers: [{
                    ...
                    icon: {image: "js/images/mapicon.png", iconsize: [26, 47],},
                    key: 'key1'
                }],
                scrollwheel: false,
        ...
        if ($('form[name="frmnewsletter"]').length) {
            $('form[name="frmnewsletter"]').submit(function () {
                ...
                if ($(This).valid()) {
                    ...
                    $.ajax({
                        type: "POST", url: action, data: data_value, error: function (xhr, status, error) {
                            confirm('Something went wrong!');
            ...
            $('form[name="frmnewsletter"]').validate({
                rules: {mc_email: {required: true, email: true}},
    ...
    jQuery(window).load(function () {
        ...
        function chart() {
            var $ = jQuery;
            $('.chart').each(function () {
                var $this = $(this), line = [], type = 'line', width = '100%', height = '225', lineColor = '#e1e1e1',
                    fillColor = 'rgba(0, 0, 0, .05)', spotColor = '#a9a8a8', minSpotColor = '#c6c6c6',
                    maxSpotColor = '#727070', verticalLineColor = '#e1e1e1', spotColorHovered = '#1e1e1e',
                    lineWidth = 2, barSpacing = 8, barWidth = 18, barColor = 'rgba(0, 0, 0, .2)', offset = 0,
                    sliceColors = [], colorMap = [], rangeColors = ['#d3dafe', '#a8b6ff', '#7f94ff'],
                    posBarColor = '#c6c6c6', negBarColor = '#727070', zeroBarColor = '#a9a8a8',
                    performanceColor = '#575656', targetWidth = 5, targetColor = '#1e1e1e';
        ...
        function graph($re) {
            ...
            tax_data = [{period: "2011 Q3", licensed: 3407, sorned: 660}, {
                period: "2011 Q2",
                licensed: 3351,
                sorned: 629
            ...
            if ($('#hero-donut').length) {
                Morris.Donut({
                    element: "hero-donut",
                    data: [{label: "Acrylic Art", value: 80}, {label: "Sculpture", value: 30}, {
                        label: "Dropper Art",
                        value: 60
                    }, {label: "Sketches", value: 70}],
        ...
        if ($('#diagram').length) {
            var o = {
                init: function () {
                    ...
                    var r = Raphael('diagram', 320, 320), rad = 20, defaultText = 'Skills', speed = 250;
                    r.circle(160, 160, 40).attr({stroke: 'none', fill: '#193340'});
                    var title = r.text(160, 160, defaultText).attr({font: '12px Arial', fill: '#fff'}).toFront();
                    r.customAttributes.arc = function (value, color, rad) {
                        var v = 3.6 * value, alpha = v == 360 ? 359.99 : v, random = o.random(91, 240),
                            a = (random - alpha) * Math.PI / 180, b = random * Math.PI / 180,
                            sx = 160 + rad * Math.cos(b), sy = 160 - rad * Math.sin(b), x = 160 + rad * Math.cos(a),
                            y = 160 - rad * Math.sin(a),
                            path = [['M', sx, sy], ['A', rad, rad, 0, +(alpha > 180), 1, x, y]];
                        return {path: path, stroke: color}
                    }
                    $('.get').find('.arc').each(function (i) {
                        var t = $(this), color = t.find('.color').val(), value = t.find('.percent').val(),
